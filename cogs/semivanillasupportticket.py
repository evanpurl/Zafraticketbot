...
class closemodal(ui.Modal, title='Semi-Vanilla Support Ticket Closure'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        lchanid = await dbgetlogchannel("Semi-Vanilla Support")
...
class autoclosemodal(ui.Modal, title='Semi-Vanilla Support Ticket Closure'):
    reason = ui.TextInput(label='PLEASE GIVE A REASON FOR CLOSING THIS TICKET.', style=discord.TextStyle.paragraph,
                          max_length=600)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if ticketcat:
            ticketchan = await interaction.guild.create_text_channel(
                f"ticket-{interaction.user.name}-semivanillasupport", category=ticketcat,
            ...
            ticketchan = await interaction.guild.create_text_channel(
                f"ticket-{interaction.user.name}-semivanillasupport", overwrites=overwrites)
            overwrite = discord.PermissionOverwrite()
            overwrite.send_messages = False
            for role in rolelist:
                await ticketchan.set_permissions(discord.utils.get(interaction.guild.roles, name=role),
...
class ticketbuttonpanel(discord.ui.View):
    ...
    @discord.ui.button(label="Close Ticket", emoji="🗑️", style=discord.ButtonStyle.red,
                       custom_id="semivanillasupport:close")
    ...
    @discord.ui.button(label="Auto-Close Ticket", emoji="⏲️", style=discord.ButtonStyle.gray,
                       custom_id="semivanillasupport:autoclose")
...
class ticketbutton(discord.ui.View):
    ...
    @discord.ui.button(label="Create Ticket", emoji="📨", style=discord.ButtonStyle.blurple,
                       custom_id="semivanillasupportbutton")
    async def gray_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            existticket = discord.utils.get(interaction.guild.channels,
                                            name=f"ticket-{interaction.user.name.lower()}-semivanillasupport")
            if existticket:
                await interaction.response.send_message(
...
def ticketmessageembed(bot):
    embed = discord.Embed(title="**Semi-Vanilla Support Tickets**",
